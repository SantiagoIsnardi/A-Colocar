"""
Uso:
    python -m app.cli.seed_sofascore "Argentina Liga Profesional" 2024 10

El tercer parámetro (opcional) limita cuántos partidos procesar en esta
corrida — recomendable empezar con un número chico (5-10) para la primera
prueba real, ya que cada partido tarda varios segundos (Selenium + Chrome).
"""
...
async def main(league: str, year: str, max_matches: int | None) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        result = await service.ingest_league_season(
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print('Uso: python -m app.cli.seed_sofascore "<liga>" <año> [max_partidos]')
        sys.exit(1)
    _league = sys.argv[1]
    _year = sys.argv[2]
